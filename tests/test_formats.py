...
class TestPrettyFormat:
    ...
    def test_parse_with_explict_type_parsers(self) -> None:
        pretty = PrettyFormat(types_location=HEADER, type_parse={'bytes': lambda text: text.encode('utf-8')})
        actual = pretty.parse(
            """
            +-+--------+
            |x|y(bytes)|
    ...
    def test_parse_with_explict_column_parsers(self) -> None:
        pretty = PrettyFormat(column_parse={'x': int, 'y': lambda text: text.encode('utf-8')})
        actual = pretty.parse(
            """
            +-----+------+
            | x   | y    |
    ...
    def test_render_with_explict_column_renderers(self) -> None:
        pretty = PrettyFormat(column_render={'x': lambda f: f'{f:.0f}', 'y': lambda b: b.decode('utf-8')})
        actual = pretty.render(
            [
                {'x': 1.1, 'y': b'foo'},
                {'x': 2.2, 'y': b'bar'},
...
class TestCSVFormat:
    ...
    def test_render_empty(self) -> None:
        ...
        actual = format_.render([])
        compare(actual, expected=dedent(""))
